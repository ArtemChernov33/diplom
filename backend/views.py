...
from .models import Shop, Category, ProductInfo, Product, Parameter, ProductParameter, ConfirmEmailToken, Contact, \
    OrderItem, Order
...
class RegisterAccount(APIView):
    ...
    Для регистрации пользователя необходимо указать следующие поля:
        first_name,
        last_name,
        email,
        password,
    ...
    После этого на указанный email приходит токен для подтверждения"""
...
class ConfirmAccount(APIView):
    ...
    Для подтверждения почтового адреса необходимо передать токен,
...
class AccountDetails(APIView):
    ...
    """Для авторизации необходимо указать уникальный токен"""
    ...
    """
        Передаем данные, которые хотим изменить например:
        first_name,
        last_name,
        email,
        password,
...
class LoginAccount(APIView):
    ...
    Для авторизации необходимо передать свой email и пароль,
    что бы получить уникальный токен для авторизации в системе.
    """
    def post(self, request, *args, **kwargs):
...
class ContactView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        """Для авторизации необходимо указать уникальный токен
        Для создания контактной информации по пользователю, необходимо передать следующие параметры:
        city,
        street,
        house,
        structure,
    ...
    def put(self, request, *args, **kwargs):
        """Для авторизации необходимо указать уникальный токен
        Для изменения контактной информации по пользователю, необходимо передать следующие параметры:
        city,
        street,
        house,
        structure,
    ...
    def delete(self, request, *args, **kwargs):
        """Для авторизации необходимо указать уникальный токен
        Для удаления контактной информации по пользователю, необходимо передать следующие параметры:
        "items" : "id"
        где id это номер контактной информации
...
class PartnerUpdate(APIView):
    ...
    Для загрузки прайса  необходима авторизация от поставщика.
    В запросе указываем url ".yml"файла (размещенного в сети интернет)
    """
    def post(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            return JsonResponse({'Status': False, 'Error': 'Log in required'}, status=403)
...
class BasketView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        """Для добавления товаров в  корзину необходимо указать:
        ...
        if items_sting:
            try:
                ...
                return JsonResponse({'Status': True, 'Add in basket': objects_created})
        return JsonResponse({'Status': False, 'Errors': 'Not all the required are provided'})

    def delete(self, request, *args, **kwargs):
        """Для удаления из корзины товаров, необходимо указать их id:"""
        if not request.user.is_authenticated:
            return JsonResponse({'Status': False, 'Error': 'Log in required'}, status=403)
        items_sting = request.data.get('items')
        print(items_sting)
...
class OrderView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        """Для размещения заказа изи корзины необходимо передать:
        id (заказа)
        status (должен быть basket)
        contact (покупателя)
        """
        if not request.user.is_authenticated:
            return JsonResponse({'Status': False, 'Error': 'Log in required'}, status=403)
